...
To run rtrcheck in a docker container, first build the container:

```text
docker build -t lukastribus/rtrcheck .
```

Then you can use the following command:

```text
docker run --rm -v /tmp/rtrcheck:/data lukastribus/rtrcheck --help
```

You can alias this command in your shell for easier usage:

```text
alias rtrcheck='docker run --rm -v /tmp/rtrcheck:/data lukastribus/rtrcheck'
...
* shellcheck tested
* exit on failure with appropriate nagios-friendly exit codes
